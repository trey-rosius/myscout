...
class NotificationItem extends StatelessWidget {
  NotificationItem({this.userId,this.document});
  final String userId;
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
        ...
        .document(document[Config.senderId])
        .snapshots(),
    ...
    if (docSnap.hasData)
    {
      ...
      return InkWell(
        onTap: (){
          if(document[Config.notificationType] == Config.comment)
            {
              Navigator.push(
                  context,
                  new MaterialPageRoute(
                    builder: (context) => CommentsScreen(
                      ...
                      postAdminId: document[Config.receiverId],
                    ),
                  ));
            }
            else if(document[Config.notificationType] == Config.like)
              {
                Navigator.push(
                    context,
                    new MaterialPageRoute(
                      builder: (context) => ViewCompletePost(
                        userId: userId,
                        postId: document[Config.postId],
                 ...
                 Expanded(
                   child: Wrap(
                     children: <Widget>[
                       ...
                       Padding(
                         padding: const EdgeInsets.only(right:3.0),
                         child: Text(document[Config.notificationText]+".",style: TextStyle(fontSize: 17.0,color: Theme.of(context).primaryColor),),
                       ),
